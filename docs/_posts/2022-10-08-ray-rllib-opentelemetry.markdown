...
The default RLlib metric didn't indicates how long it spent sampling the environment. Before Ray v2.0, it provided `sample_time_ms` but the value doesn't make sense if we just add it together wth `learn_time_ms`. This is because the sum didn't equal to the `training_iteration_time_ms`.
...
The setup function will be executed when initialising each Ray local/remote worker. It will configure the span processor and traces exporter. In the example, it is configured to send the traces to a local Jaeger instances. The local Jaeger instances is created through a [docker-compose.yaml](https://github.com/Wal8800/card-games/blob/main/docker-compose.yaml) and we can view the traces at `localhost:16686` from Jaeger web interface.
...
After the training run is completed, we can go to the Jaeger web interface and view the spans for the traces we did for training.
...
- There are 4 `RolloutWorker.sampel.ray.remote_worker` spans running at the same time. This indicates there are 4 workers sampling in parallel which matches with the training configuration. 
...
- The sampling span `RolloutWorker.sample.ray.remote` duration is shorter than it's child span `RolloutWorker.sampel.ray.remote_worker`. Most likely due to asynchronous execution where the sampling span was the function that triggers the worker function to sample and exit after triggering the worker. The worker continues to sample until it meets the sampling target.  
...
We can add further instrumentations in the training step by creating a custom `train_one_step` and `do_minibatch_sgd` function. Then we can see what it is doing.
...
With this view, we are now able to see any impacts on time taken when changing the configurations or customising the implementation for RLlib PPO training. 
...
In the self implemented PPO training, the number of update iteration is set to 80. Often times, the number of iteration that got executed is significiant less than 80 due to early stopping. Usually around less than 10 update iterations. 

On the other hand, when I first tried RLlib PPO training, I kept the training hyperparameters the same. This means the the number of update iteration `num_sgd_iter` is set to 80 as well. Since the RLlib PPO implementation doesn't have early stopping, it always ran 80 update iterations. As a result, this contributes to making the RLlib PPO training taking longer than the self impemented PPO training.

To enable early stopping in RLlib PPO training, I need to change the existing RLlib PPO training logic. Luckily, since I'm already customising the `do_minibatch_sgd` function to enable spans so I implemented the same early stopping logic in there. After training with early stopping, the number of iteration drops to ~30 iterations and it reduce the time taken for updating the agent. However, the number of iterations are still more than the self implemented PPO training. 

After further investigation,  it looks like RLlib PPO implementation uses [both KL penalty and clipping simultaneously](https://github.com/ray-project/ray/issues/13837) where as my implementation only uses clipping. This means potentially, for the RLlib PPO implementation, the mean KL divergence between old policy to new policy in a single update iteration is relatively less than my PPO implementation. Consequently, RLlib PPO training performs more iteration until it reached the same early stopping threshold.
...
Overall, after enabling eager tracing and changing the number of update iteration to a lower value, the time to train a single epoch is much faster now. Especially for `train_one_step`, the time taken decrease from ~22 seconds to ~7 seconds.  

The RLlib PPO agent update is relatively slower than self implemented PPO training. However, due to the faster sampling in RLlib, the overall RLlib PPO training is at least the same or faster than self implemented PPO training. Furthermore, from initial experiments, the agent trained from RLlib PPO is peforming better compared to self implemented PPO training when trained with the same number of epoch.
